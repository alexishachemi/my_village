...
{
    snprintf(buf, NAME_SIZE, PROP_CHILD_NAME_FORMAT, parent->name, x, y);
    return buf;
}
    ...
    u_int8_t x, u_int8_t y)
{
    ...
    if (!prop || !map || x == 0 || y == 0 || prop_get_child(prop, x, y))
        return NULL;
    child = new_child(prop);
    if (!prop_init(child, generate_child_name(prop, child_name, x, y)))
        return NULL;
...
prop_t *prop_get_child(prop_t *prop, u_int8_t x, u_int8_t y)
{
    ...
    for (unsigned int i = 0; i < REG_SIZE(prop->childs); i++) {
        ...
        if (child && child->offx == x && child->offy == y)
            return child;
    }
    return NULL;
}
...
Test(prop, add_child)
{
    ...
    child = prop_add_child(&prop, &map, 1, 1);
    cr_assert_not_null(child);
    cr_assert_null(prop_add_child(&prop, &map, 1, 1));
    cr_assert_eq(REG_SIZE(prop.childs), 1);
    cr_assert_eq(child, REG_AT(prop_t, &prop.childs, 0));
    child2 = prop_add_child(&prop, &map, 1, 2);
    ...
    cr_assert_eq(child->offx, 1);
    cr_assert_eq(child->offy, 1);
...
Test(prop, get_child)
{
    ...
    cr_assert(prop_add_child(&prop, &map, 1, 1));
    cr_assert_eq(REG_SIZE(prop.childs), 1);
    child = prop_get_child(&prop, 1, 1);
    cr_assert(child);
    cr_assert_eq(child->type, PTYPE_CHILD);
    cr_assert_eq(child->offx, 1);
    cr_assert_eq(child->offy, 1);
    cr_assert_eq(child->parent, &prop);
    cr_assert_eq(child->asset_map.asset_mode, AMODE_NONE);
    cr_assert_eq(prop_get_child(&prop, 0, 0), &prop);
    cr_assert_null(prop_get_child(&prop, 1, 0));
    cr_assert_null(prop_get_child(&prop, 0, 1));
}
